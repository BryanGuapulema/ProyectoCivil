        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 sm:px-20 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <form method="POST" action="{{ route('rubros_kg.update', $rubro->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select id="nombre" name="nombre" required class="block mt-1 w-full dark:bg-gray-900 dark:text-gray-300">
                                <option value="Acero longitudinal en columnas" {{ $rubro->nombre == 'Acero longitudinal en columnas' ? 'selected' : '' }}>Acero longitudinal en columnas</option>
                                <option value="" disabled>Seleccione un nombre</option>
                                <option value="Acero longitudinal en vigas" {{ $rubro->nombre == 'Acero longitudinal en vigas' ? 'selected' : '' }}>Acero longitudinal en vigas</option>
                                <option value="Acero transversal en columnas" {{ $rubro->nombre == 'Acero transversal en columnas' ? 'selected' : '' }}>Acero transversal en columnas</option>
                                <option value="Acero transversal en vigas" {{ $rubro->nombre == 'Acero transversal en vigas' ? 'selected' : '' }}>Acero transversal en vigas</option>
                                <option value="Acero de refuerzo en losa" {{ $rubro->nombre == 'Acero de refuerzo en losa' ? 'selected' : '' }}>Acero de refuerzo en losa</option>
                                <option value="Acero estructural en columna" {{ $rubro->nombre == 'Acero estructural en columna' ? 'selected' : '' }}>Acero estructural en columna</option>                                
                                <option value="Acero estructural en viga" {{ $rubro->nombre == 'Acero estructural en viga' ? 'selected' : '' }}>Acero estructural en viga</option>                                
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="denominacion" name="denominacion" required class="block mt-1 w-full dark:bg-gray-900 dark:text-gray-300">
                                ...
                                <option value="Vr ⌀ 8" @if($rubro->denominacion == 'Vr ⌀ 8') selected @endif>Vr ⌀ 8</option>
                                <option value="Vr ⌀ 10" @if($rubro->denominacion == 'Vr ⌀ 10') selected @endif>Vr ⌀ 10</option>
                                <option value="Vr ⌀ 12" @if($rubro->denominacion == 'Vr ⌀ 12') selected @endif>Vr ⌀ 12</option>
                                <option value="Vr ⌀ 14" @if($rubro->denominacion == 'Vr ⌀ 14') selected @endif>Vr ⌀ 14</option>
                                <option value="Vr ⌀ 16" @if($rubro->denominacion == 'Vr ⌀ 16') selected @endif>Vr ⌀ 16</option>
                                <option value="Vr ⌀ 18" @if($rubro->denominacion == 'Vr ⌀ 18') selected @endif>Vr ⌀ 18</option>
                                <option value="Vr ⌀ 20" @if($rubro->denominacion == 'Vr ⌀ 20') selected @endif>Vr ⌀ 20</option>
                                <option value="Vr ⌀ 22" @if($rubro->denominacion == 'Vr ⌀ 22') selected @endif>Vr ⌀ 22</option>
                                <option value="HEB 240" @if($rubro->denominacion == 'HEB 240') selected @endif>HEB 240</option>
                                <option value="HEB 280" @if($rubro->denominacion == 'HEB 280') selected @endif>HEB 280</option>
                                <option value="HEB 300" @if($rubro->denominacion == 'HEB 300') selected @endif>HEB 300</option>
                                <option value="TC150x150x3" @if($rubro->denominacion == 'TC150x150x3') selected @endif>TC150x150x3</option>
                                <option value="TC150x150x4" @if($rubro->denominacion == 'TC150x150x4') selected @endif>TC150x150x4</option>
                                <option value="TC150x150x5" @if($rubro->denominacion == 'TC150x150x5') selected @endif>TC150x150x5</option>
                                <option value="TC200x200x3" @if($rubro->denominacion == 'TC200x200x3') selected @endif>TC200x200x3</option>
                                <option value="TC200x200x4" @if($rubro->denominacion == 'TC200x200x4') selected @endif>TC200x200x4</option>
                                <option value="TC200x200x5" @if($rubro->denominacion == 'TC200x200x5') selected @endif>TC200x200x5</option>
                                <option value="TC300x300x3" @if($rubro->denominacion == 'TC300x300x3') selected @endif>TC300x300x3</option>
                                <option value="TC300x300x4" @if($rubro->denominacion == 'TC300x300x4') selected @endif>TC300x300x4</option>
                                <option value="TC300x300x5" @if($rubro->denominacion == 'TC300x300x5') selected @endif>TC300x300x5</option>
                                <option value="TR150x100x3" @if($rubro->denominacion == 'TR150x100x3') selected @endif>TR150x100x3</option>
                                <option value="TR150x100x4" @if($rubro->denominacion == 'TR150x100x4') selected @endif>TR150x100x4</option>
                                <option value="TR200x100x4" @if($rubro->denominacion == 'TR200x100x4') selected @endif>TR200x100x4</option>
                                <option value="TR250x150x5" @if($rubro->denominacion == 'TR250x150x5') selected @endif>TR250x150x5</option>
                                <option value="TR300x200x5" @if($rubro->denominacion == 'TR300x200x5') selected @endif>TR300x200x5</option>
                                <option value="2C200x100x6" @if($rubro->denominacion == '2C200x100x6') selected @endif>2C200x100x6</option>
                                <option value="2C250x100x6" @if($rubro->denominacion == '2C250x100x6') selected @endif>2C250x100x6</option>
                                <option value="2C300x100x6" @if($rubro->denominacion == '2C300x100x6') selected @endif>2C300x100x6</option>
                                <option value="2C300x150x10" @if($rubro->denominacion == '2C300x150x10') selected @endif>2C300x150x10</option>
                                <option value="IPE 120" @if($rubro->denominacion == 'IPE 120') selected @endif>IPE 120</option>
                                <option value="IPE 140" @if($rubro->denominacion == 'IPE 140') selected @endif>IPE 140</option>
                                <option value="IPE 180" @if($rubro->denominacion == 'IPE 180') selected @endif>IPE 180</option>
                                <option value="IPE 200" @if($rubro->denominacion == 'IPE 200') selected @endif>IPE 200</option>
                                <option value="IPE 220" @if($rubro->denominacion == 'IPE 220') selected @endif>IPE 220</option>
                                <option value="IPE 240" @if($rubro->denominacion == 'IPE 240') selected @endif>IPE 240</option>
                            </select>
                        </div>

                        <div class="mb-4">
                            <x-label for="longitud" value="{{ __('longitud (h)') }}" class="text-gray-700 dark:text-gray-300"/>
                            <x-input id="longitud" type="number" step="0.01" value="{{ $rubro->longitud }}" name="longitud" required class="mt-1 block w-full dark:bg-gray-900 dark:text-gray-300"/>
                        </div>

                        <div class="mb-4">
                            <x-label for="cantidad" value="{{ __('Cantidad (u)') }}" class="text-gray-700 dark:text-gray-300"/>
                            <x-input id="cantidad" type="number" step="1" value="{{ $rubro->cantidad }}" name="cantidad" required class="mt-1 block w-full dark:bg-gray-900 dark:text-gray-300"/>
                        </div> 

                        <div class="mb-4">
                            <x-label for="kgm" value="{{ __('kgm (h)') }}" class="text-gray-700 dark:text-gray-300"/>
                            <x-input readonly id="kgm" type="number" step="0.01" value="{{ $rubro->kgm }}" name="kgm" required class="mt-1 block w-full dark:bg-gray-900 dark:text-gray-300"/>
                        </div>
                        
                        <div class="mb-4">
                            <x-label for="peso" value="{{ __('peso (h)') }}" class="text-gray-700 dark:text-gray-300"/>
                            <x-input id="peso" type="number" step="0.01" value="{{ $rubro->peso }}" name="peso" required class="mt-1 block w-full dark:bg-gray-900 dark:text-gray-300"/>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @if($rubro->evidencia)
                                <img src="{{ asset('storage/evidencias/rubroskg/' . $rubro->evidencia) }}" alt="Evidencia actual" class="mt-4 rounded-lg">
                            @endif
                        </div>
